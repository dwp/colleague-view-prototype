...
router.post('/index.html', function (req, res) {
  ...
  res.redirect('/extra-scenarios/dla-type-selection/index');
});
...
router.post('/why-checking-information', function (req, res) {
  ...
  if (
    ...
    res.redirect('/extra-scenarios/dla-type-selection/why-checking-info');
  } else {
    // var errMsg = '';
    if (
      ...
      res.redirect('/extra-scenarios/dla-type-selection/error-nino-number');
    } else {
      res.redirect('back');
    }
  }
...
router.post('/call-with', function (req, res) {
  ...
  if (req.session.data['What-type-of-contact'] == 'Telephone call with') {
    res.redirect('/extra-scenarios/dla-type-selection/call-with');
  }
  if (req.session.data['What-type-of-contact'] == 'Contact with') {
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/contact-with');
  }
  if (req.session.data['What-type-of-contact'] == 'View only') {
    res.redirect('/extra-scenarios/dla-type-selection/view-only/home');
  }
  if (req.session.data['What-type-of-contact'] == 'Log a contact attempt') {
    res.redirect('/extra-scenarios/dla-type-selection/add-call/what-benefits-discussed');
  }
  if (req.session.data['What-type-of-contact'] == '') {
    console.log('Error page');
    res.redirect('/extra-scenarios/dla-type-selection/error-why-checking-info');
  }
});
...
router.post('/home', function (req, res) {
  ...
  if (req.session.data['Who-is-the-phone-call-with-ur8'] == '') {
    ...
    res.redirect('/extra-scenarios/dla-type-selection/home');
  }
});
...
router.post('/non-telephony/home', function (req, res) {
  ...
  if (req.session.data['Who-is-contact-with'] == '') {
    // Send user to error page
    res.redirect(
      '/extra-scenarios/dla-type-selection/non-telephony/contact-with'
    );
  } else {
    if (
    ...
    res.redirect(
      '/extra-scenarios/dla-type-selection/non-telephony/home'
    );
  }
});
...
router.post('/add-call/what-benefits-discussed', function (req, res) {
  if (req.session.data['what-benefit-discussed'] != '') {
    ...
    res.redirect('/extra-scenarios/dla-type-selection/add-call/do-you-want-add-more-details');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    req.session.data['addNote']= '';
...
router.post('/add-call/questions-asked', function (req, res) {
    ...
    {
      req.session.data['addNote'] = '';
      res.redirect('/extra-scenarios/dla-type-selection/add-call/add-note-for-failed-outbound');
    } else
    {
   ...
   {
    console.log('Error page');
    res.redirect('/extra-scenarios/dla-type-selection/add-call/what-benefits-discussed-error');
    } else if (req.session.data['what-benefit-discussed'] == 'General information') {
      res.redirect('/extra-scenarios/dla-type-selection/add-call/no-benefit');
    } else {
    res.redirect('/extra-scenarios/dla-type-selection/add-call/questions-asked');
   }
  }
})
...
router.post('/add-call/questions-answered', function (req, res) {
  res.redirect('/extra-scenarios/dla-type-selection/add-call/questions-answered');
});
...
router.post('/add-call/is-question-resolved', function (req, res) {
  ...
  res.redirect('/extra-scenarios/dla-type-selection/add-call/is-question-resolved');
});

router.post('/add-call/added-call-details', function (req, res) {
  
  res.redirect('/extra-scenarios/dla-type-selection/add-call/added-call-details');
});
...
router.post('/add-call/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    ...
    res.redirect('/extra-scenarios/dla-type-selection/add-call/add-note')
  } else{
    console.log('This is Newcastle');
    res.redirect('/extra-scenarios/dla-type-selection/add-call/added-details');
  }
});
...
router.post('/add-call/add-more-call-details', function (req, res) {
  ...
  if (addCallDetails == 'Complete phone call'){
    // res.redirect('/extra-scenarios/dla-type-selection/add-call/call-completed');
    res.redirect('/extra-scenarios/dla-type-selection/index')
  }
  if (addCallDetails == 'Change your note'){
    res.redirect('/extra-scenarios/dla-type-selection/add-call/change-notes');
  }
  if (addCallDetails == 'Add a note'){
    res.redirect('/extra-scenarios/dla-type-selection/add-call/add-a-note');
  }
});
...
router.post('/add-call/do-you-want-to-complete-call', function (req, res) {
  if (req.session.data['what-benefit-discussed'] != '') {
    res.redirect('/extra-scenarios/dla-type-selection/add-call/do-you-want-to-complete-call');
  }
  else{
    req.session.data['what-benefit-discussed'] = '';
    res.redirect('/extra-scenarios/dla-type-selection/add-call/no-contact-added');
  }
});
...
router.post('/non-telephony/add-contact/what-benefits-discussed', function (req, res) {
  if (req.session.data['what-benefit-discussed-non-telephony'] != '') {
    // console.log('Display summary page');
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/do-you-want-add-more-details');
  }
  else{
    // req.session.data['what-benefit-discussed-non-telephony'] = '';
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/what-benefits-discussed');
  }
});
...
router.post('/non-telephony/add-contact/questions-asked', function (req, res) {
 ...
 {
  console.log('Error page');
  res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/what-benefits-discussed-error');
 } else if (req.session.data['what-benefit-discussed-non-telephony'] == 'General information') {
  res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/no-benefit');
} else {
  res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/questions-asked');
 }
})
...
router.post('/non-telephony/add-contact/questions-answered', function (req, res) {
  res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/questions-answered');
});
...
router.post('/non-telephony/add-contact/is-question-resolved', function (req, res) {
  ...
  req.session.data['chpa_question-resolved']= '';
  req.session.data['othQ_question-resolved']= '';
  res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/is-question-resolved');
});

router.post('/non-telephony/add-contact/added-contact-details', function (req, res) {
  
  res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/added-contact-details');
});
...
router.post('/non-telephony/add-contact/added-details', function (req, res) {
  ...
  if(addNote =='Yes'){
    console.log('Add Notes');
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/add-note')
  } else{
    console.log('This is Newcastle');
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/added-details');
  }
});

router.post('/non-telephony/add-contact/check-before-start-sercive-again', function (req, res) {
  if (req.session.data['what-benefit-discussed-non-telephony'] == ''){
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/check-before-start-sercive-again');
  } else{
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/do-you-want-to-complete-contact'); 
  }
});
...
router.post('/non-telephony/add-contact/contact-details-added', function (req, res) {
  ...
  if (addContactDetails == 'Yes'){
    ...
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/contact-completed')
  }
});
...
router.post('/non-telephony/add-contact/complete-contact', function (req, res) {
  ...
  if (checkCallCompletion == 'Complete phone contact'){
    ...
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/home')
  }
...
router.post('/non-telephony/add-contact/add-more-contact-details', function (req, res) {
  ...
  if (addCallDetails == 'Change your note'){
    ...
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/change-notes')
  }
  if (addCallDetails == 'Add a note'){
    console.log('Add a note');
    res.redirect('/extra-scenarios/dla-type-selection/non-telephony/add-contact/add-a-note')
  }
});
